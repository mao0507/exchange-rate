/** 原生 select 共用表面樣式（與 prompt.xml / 全站 token 對齊） */
const base =
  'appearance-none rounded-lg text-fg bg-bg-input border border-white/10 shadow-inner ' +
  'cursor-pointer ' +
...
/** 換算區全寬幣別：觸控目標約 44px */
export const selectFieldMd = `${base} w-full min-h-[44px] px-4 py-3 pr-10 text-base`

/** 歷史幣別對：等寬字；字級至少 16px 以避免 iOS 聚焦縮放 */
export const selectFieldSm = `${base} min-h-[44px] min-w-0 pl-3 pr-8 py-2 text-base font-mono`
